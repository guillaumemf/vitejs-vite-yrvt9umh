...
export default function Chapardeur() {
  ...
  const [buildings, setBuildings] = useState({ basket: 0 });
  const [floats, setFloats] = useState([]);
  const [showAct2Banner, setShowAct2Banner] = useState(false);
  const [act2Unlocked, setAct2Unlocked] = useState(false);
  const [clickAnim, setClickAnim] = useState(false);

  const eurosRef = useRef(euros);
  const totalRef = useRef(totalEuros);
  const strawRef = useRef(strawberries);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const buildingProd =
        BUILDINGS.find((b) => b.id === "basket").baseProduction * basketCount;
    ...
    >
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: "0.7rem", color: "#a3714a", textTransform: "uppercase", letterSpacing: "0.1em" }}>
              Chapardeur
            </div>
            <div style={{ fontWeight: "bold", fontSize: "1rem" }}>{pseudo}</div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: "1.4rem", fontWeight: "bold", color: "#f97316" }}>
              {formatEuros(euros)}
            </div>
      ...
      {/* CLICK ZONE */}
      <div
        style={{ padding: "1.5rem 1rem 1rem", textAlign: "center" }}
      >
        ...
        <div style={{ position: "relative", display: "inline-block" }}>
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              animation: clickAnim
                ? "clickPop 0.1s ease, pulse-glow 2s ease-in-out infinite"
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "0.75rem", color: "#a3714a" }}>
                    {b.desc}
                    {count > 0 && (
                      <span style={{ color: "#f97316" }}> · Total : {formatEuros(totalProd)}/sec</span>
                    )}
                  </div>
                </div>
                <button
